...
for gamma=Gamma
    ...
    rel_y=1;
    %%%%%%%
    while rel_y > Tol && count_ADMM < MaxIter
    ...
    %%%%% DR
    q_0_DR= (z_0_DR-gamma/(gamma+1)*Y)*(gamma+1);
 ...
 %gamma=1.2;
    while rel_y > Tol && count_DR < MaxIter
            ...
            % GAUSSIAN
            z_tplus1=(Y+1/gamma*ylambda_k)/(1+1/gamma);
    ...
    hold on
    semilogy(1:count_ADMM-1,relative_ADMM_y(trial,5:count_ADMM+3),'r')
