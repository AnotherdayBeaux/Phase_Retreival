...
function    [X,new_mask] =ptycho_rand_ifft_unmask(Y,pre_X,Na,Nb,os_rate,num_mask,mask,l_patch,x_c_p,y_c_p, p_c,alpha)
  half_patch= (l_patch-1)/2;
  Big_X=zeros(3*Na, 3*Nb);
  Big_pre_X=kron(ones(3,3),pre_X);
  ...
  switch p_c
     ...
     for  i = 1:subNa
         ...
         for j = J_vec(rev+1,:)
             ...
             new_mask=mask+alpha*conj(piece_X)./(abs(piece_X).^2).*(piece_X-Big_pre_X(center_x-half_patch:center_x+half_patch, center_y-half_patch: center_y+half_patch));
