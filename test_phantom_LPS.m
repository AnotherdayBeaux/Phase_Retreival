...
%%%%     1. test effect of   sum (a_j linear phase shift_j) in Phantom case
%%%%     6/2/2018
...
% patch.
    ...
    Bigname='/Users/Beaux/Desktop/phase retrieval/code/16/combination of LPS RPP 2';
    mkdir(Bigname)
    
   
    
    [IM,Y,Z,NSR,phase_arg,nor_ptycho,nor_phase]=ini_rand_ptycho(str1,str2,os_rate,num_mask,coer,SUBIM,...
    type_im,l_patch,x_c_p,y_c_p,bd_con,mask_type,c_l, delta, beta_1,beta_2, rho,salt_on);
    ...
    k2=1;
    l2=0;a2=0*exp(2i*pi*rand(1));
    
    k3=0;
    l3=1;a3=0*exp(2i*pi*rand(1));
    
    k4=1;
    l4=1;a4=0*exp(2i*pi*rand(1));
    
    k5=1;
    l5=2;a5=0*exp(2i*pi*rand(1));
    ...
    l_phase_drift_mask_sum = a1*exp(-2i*pi/n*(mask_x_grid*k1+mask_y_grid*l1))+...
    ...
    %%%%%
    mask_LPS1= mask.* exp(-2i*pi/n*(mask_x_grid*k1+mask_y_grid*l1));
...
while res_x > Tol && count_DR< MaxIter
   ...
   % calculate nor_phase dependent on image
    ...
    mask_k=mask_estimate./abs(mask_estimate);
    ee_mask=norm(mask_estimate(:)'*mask_LPS1(:))/(mask_estimate(:)'*mask_LPS1(:));
    rel_mask_LPS= norm(ee_mask*mask_LPS1-mask_estimate,'fro')/norm(mask_LPS1,'fro');
    relative_DR_maskLPS(1,count_DR)=rel_mask_LPS;
    %%%% MPC enforcement.
    %mask_arg=angle(mask_estimate./mask);       % center
    %center_arg=sum(sum(mask_arg))/m/m;
    %proj_mask_arg=max(mask_arg-center_arg, -delta*pi);
    ...
    fprintf('count_DR=%d\n rel_mask_LPS=%4.3s r_s_1=%4.3s r_s_2=%4.3s r_s_3=%4.3s r_s_4=%4.3s r_s_5=%4.3s res_x=%s\n',...
                count_DR,rel_mask_LPS, rel_shift_x_1,rel_shift_x_2, rel_shift_x_3,rel_shift_x_4, rel_shift_x_5, res_x);
    count_DR=count_DR+1;
...
%%%%% get rid of linear phase


K=getrid_LPS(IM, ee*x__t);



kshift=-real(K);

shift_mask=mask_estimate.*exp(2i*pi/n*(mask_x_grid*kshift(1)+mask_y_grid*kshift(2)));


eee=norm(shift_mask(:)'*mask(:))/(shift_mask(:)'*mask(:));
rel_shift_x=norm(eee*mask-shift_mask,'fro')/norm(mask,'fro');
...
hold on
semilogy(1:count_DR-1,relative_DR_maskLPS(1,1:count_DR-1),'b.')
hold on
xlabel('iter')
ylabel(' ')
hold on
...
name_LPS=['RelErr f_0 exp(ikn)'];
name_LPS_mask = 'RelErr mu_0 exp(ikm)';
legend(name_LPS,name_res)
name=[foldername,'/convergence_LPS_phantom'];
saveas(gcf,name,'png')
close

figure
ee_mask=norm(mask_estimate_ini(:)'*mask(:),'fro')/(mask_estimate_ini(:)'*mask(:));
mask_diff=ee_mask'*mask_estimate_ini.*conj(mask);
mask_arg_diff=angle(mask_diff);
colormap('hot')
imagesc(mask_arg_diff)
colorbar
title('initial mask phase - true mask ')
name=[foldername,'/mask_initial_difference'];
saveas(gcf,name,'png')
close
...
ee=norm(shift_IM_1(:)'*x__t(:))/(shift_IM_1(:)'*x__t(:));
x_t_=ee*x__t;
phase_diff=x_t_.*conj(shift_IM_1);
ang_diff=angle(phase_diff);
figure
imshow(abs(x_t_)/255)
title('Reconstruct modular')
name=[foldername,'/recon_modular'];
saveas(gcf,name,'png')
close
    ...
    imagesc(ang_diff)
    colorbar
    title('Recon phase -  f_0 exp(ikn)')
    name=[foldername,'/recon_phase_LPS_phase'];
    saveas(gcf,name,'png')
    close
    ...
    figure
    ee_mask=norm(mask_estimate(:)'*mask_LPS1(:),'fro')/(mask_estimate(:)'*mask_LPS1(:));
    mask_diff=ee_mask'*mask_estimate.*conj(mask_LPS1);
    mask_arg_diff=angle(mask_diff);
    colormap('hot')
    imagesc(mask_arg_diff)
    colorbar
    title('recon mask phase- LPS mask phase')
    name=[foldername,'/maskphase_lps_masp_phase'];
    saveas(gcf,name,'png')
    close
    ...
    averaged_mask_ang_diff_1=sum(mask_arg_diff,1)/m;
    averaged_mask_ang_diff_2=sum(mask_arg_diff,2)/m;
    rate_mask_shift=get_cr(averaged_mask_ang_diff_1,last);
    l2=rate_mask_shift/2/pi*n;
    figure
    plot(1:m,averaged_mask_ang_diff_1,'g');
    hold on 
    plot(1:m,averaged_mask_ang_diff_2,'r');
    xlabel('mask size')
    ylabel('phase diff')
    legend('vertical','horizontal')
    
   new_mask_LPS1=mask_LPS1.* exp(-1i*(mask_x_grid*0+mask_y_grid*rate_mask_shift));
   
   
    figure
    ee_mask=norm(mask_estimate(:)'*new_mask_LPS1(:),'fro')/(mask_estimate(:)'*new_mask_LPS1(:));
    mask_diff_mod=ee_mask'*mask_estimate.*conj(new_mask_LPS1);
    mask_arg_diff_mod=angle(mask_diff_mod);
    colormap('hot')
    imagesc(mask_arg_diff_mod)
    colorbar
