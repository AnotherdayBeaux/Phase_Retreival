...
% patch.
    ...
    Bigname='/Users/Beaux/Desktop/phase retrieval/code/16/combination of LPS';
    mkdir(Bigname)
    ...
    mask_estimate= exp(2i*pi*phase_arg).* exp(3/11*2i*pi*(rand(l_patch,l_patch)-1/2)); %.*l_phase_drift_mask ;
    %%%%%
    ...
    %%%%  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%   do precise decrease before AltDR
    %%%%  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    while rel_x > Tol && count_DR< 150
     MaxIter_u_ip = 60 * (count_DR<10) + 2* (count_DR >= 10);
    ...
    resi_diff_im=1;
    while  update_im < 2 || ( update_im < MaxIter_u_ip && resi_diff_im >1e-4)
    % phase retreival applied to image
            ...
            x_t_inner=x_t./nor_ptycho;
            AAlambda=ptycho_rand_fft(x_t_inner,os_rate,num_mask,mask_estimate,l_patch,x_c_p,y_c_p,bd_con);
            y_tplus1=AAlambda;
   ...
   % calculate nor_phase dependent on image
    ...
    fft_phase=pr_phase_fft(new_mask,os_rate,num_mask,x__t,l_patch,x_c_p,y_c_p);
    end
    
    
    residual_xx=zeros(1,MaxIter_u_ip+2);
    resi_diff_phase=1;
    while  update_phase < 2 || ( update_phase < MaxIter_u_ip && resi_diff_phase > 1e-4)
            mask_tplus1=pr_phase_ifft(fft_phase,os_rate,num_mask,x__t,l_patch,x_c_p,y_c_p);
            %mask_tplus1=nor_mask_tplus1./abs(nor_mask_tplus1);
            mask_tplus1 = mask_tplus1./abs(mask_tplus1);
            fft_phase_1=pr_phase_fft(mask_tplus1,os_rate,num_mask,x__t,l_patch,x_c_p,y_c_p);
            ...
            mask_estimate=pr_phase_ifft(fft_phase,os_rate,num_mask,x__t,l_patch,x_c_p,y_c_p);
            mask_estimate = mask_estimate./abs(mask_estimate);
            
            ee=norm(mask(:)'*mask_estimate(:))/(mask(:)'*mask_estimate(:));
            rel_mask=norm(ee*mask_estimate - mask,'fro')/norm(mask,'fro');
...
while rel_x > Tol && count_DR< MaxIter
    ...
    % phase retreival applied to image
            ...
            res_x= norm(abs(Y)-abs(y_tplus1),'fro')/norm_Y;
            Im_update = ptycho_rand_ifft(lambda_t,Na,Nb,os_rate,num_mask,mask_estimate,l_patch,x_c_p,y_c_p,p_c)./nor_ptycho;
            %fprintf('u_i=%d\n rel_x=%f\n res_x=%f\n', update_im,rel_xx,res_x);
   ...
   % calculate nor_phase dependent on image
    ...
    fft_phase=pr_phase_fft(nor_mask./sqrt(nor_phase),os_rate,num_mask,x__t,l_patch,x_c_p,y_c_p);
            ...
            nor_mask_tplus1=pr_phase_ifft(fft_phase,os_rate,num_mask,x__t,l_patch,x_c_p,y_c_p);
            %mask_tplus1=nor_mask_tplus1./abs(nor_mask_tplus1);
            mask_tplus1=nor_mask_tplus1./abs(nor_mask_tplus1);
            %nor_mask_tplus1= sqrt(nor_phase).*mask_tplus1;
            fft_phase_1=pr_phase_fft(nor_mask_tplus1,os_rate,num_mask,x__t,l_patch,x_c_p,y_c_p);
    ...
    mask_estimate=pr_phase_ifft(fft_phase,os_rate,num_mask,x__t,l_patch,x_c_p,y_c_p)./nor_phase;
    mask_estimate= mask_estimate./abs(mask_estimate);
    ...
    ee=norm(IM(:)'*x__t(:))/(IM(:)'*x__t(:));
    rel_x=norm(ee*x__t-IM,'fro')/norm(IM,'fro'); % rel error phase adj
...
rate_rel_shift=get_cr(log(rel_shift(1:count_DR-1,1)),last);
...
name_LPS=['rel err LPS x_0 No.',num2str(num_kl)];
legend('relerr x_0, rate= ',name_LPS,name_res)
