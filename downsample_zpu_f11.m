...
Bigname='/Users/Beaux/Desktop/phase retrieval/code/11/random_sample';
mkdir(Bigname);
trial_SNR=1;
...
save('Rec_err.mat','Rec_err','NSR_V','RS','RHO2')

for k=1:len_RS
    a=zeros(len_RHO2,l_d);
    for i=1:len_RHO2
        for j=1:l_d
            a(i,j)=Rec_err(j,i,k); % Rec_err=zeros(l_d,len_RHO2,len_RS);
        end
    end
    SNR_=NSR_V(k,:)   ; % NSR_V(trial_RS,trial_SNR)=SNR_DB;
    
    [x,y]=meshgrid(SNR_,RHO2);
    ...
    ylabel('rho2')
    zlabel('SNR DB')
    loc=['/Users/Beaux/Desktop/phase retrieval/code/11/random_sample/',Title];
    saveas(gcf,loc,'png')
