...
function [IM,fft2_IM,fft2_IM_pois,NSR,phase_arg,nor_ptycho,nor_phase]=ini_rand_ptycho_argfixed(str1,str2,os_rate,...
    num_mask,coer,SUBIM, type_im,l_patch,x_c_p, y_c_p,bd_con, mask_type,phase_arg_input, c_l, beta_1,beta_2, rho)        
...
switch num_mask             % 0  0 mask / Id mask
    ...
    case 1
        phase_arg=phase_arg_input;  % argument of phase
...
elseif mask_type == 1    %% correlated mask
    ...
    case 0
        ...
        mask = ones(l_patch,l_patch);
    case 1
        phase_arg=Cor_Mask(l_patch, c_l, phase_arg_input);  % argument of phase
        mask = exp(2i*pi*phase_arg);
    case 2
        phase_arg=rand(l_patch,l_patch,2);
        mask = exp(2i*pi*phase_arg);
