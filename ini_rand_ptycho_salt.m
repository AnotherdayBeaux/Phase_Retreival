%%%% ini_rand_ptycho_salt(str1,str2,os_rate, num_mask,coer,SUBIM, type_im,l_patch,x_c_p, y_c_p,bd_con, mask_type, c_l, delta, beta_1,beta_2, rho, salt_on, salt_ratio)  
...
%%% salt_on =0 no salt noise
%           =1 apply salt nose
%   salt_ratio = c each pixel has prob c to have 100 intensity salt noise

function [IM,salt_Noise,fft2_IM,fft2_IM_pois,NSR,phase_arg,nor_ptycho,nor_phase]=ini_rand_ptycho_salt(str1,str2,os_rate,...
    num_mask,coer,SUBIM, type_im,l_patch,x_c_p, y_c_p,bd_con, mask_type, c_l, beta_1,beta_2, rho, salt_on, salt_ratio)        
...
%%%%%%% add salt noise 
if salt_on == 1
   salt_Noise = rand(size(IM));
   salt_Noise(salt_Noise< (1-salt_ratio))=0;
   salt_Noise(salt_Noise> (1-salt_ratio)/2)=50+50*1i;
   IM=min(IM+salt_Noise, 255+255*1i);
end
...
elseif mask_type == 1    %% correlated mask
    switch num_mask             % 0  0 mask / Id mask
                            % 1  1 mask case
                            % 2  2 mask case 
                            % 3  1 and 1/2 mask case
    case 0
        ...
        mask = ones(l_patch,l_patch);
    case 1
        phase_arg=Cor_Mask(l_patch, c_l);  % argument of phase
        mask = exp(2i*pi*phase_arg);
    case 2
        phase_arg=rand(l_patch,l_patch,2);
        mask = exp(2i*pi*phase_arg);
