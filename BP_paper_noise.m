...
COER=[3,8,16,25,33,40,50,60,80,100];
NSR_Vec=zeros(size(COER));




gamma=1;
relative_DR_y=zeros(MaxIter,2);
Rec_Err=zeros(length(COER),2);
resi_DR_y=zeros(MaxIter,2);
...
% patch.
    ...
    Bigname='/Users/Beaux/Desktop/phase retrieval/code/19/bp_paper_figure/noise_poisson';
    mkdir(Bigname)
    
    
    
    %%%%%% running different coer which regulate noise
    count_COER=1;
    for coer=COER
...
while rel_x > Tol && count_DR< MaxIter
   ...
   % calculate nor_phase dependent on image
    ...
    resi_DR_y(count_DR,Likelihood+1)= res_x;
    relative_DR_y(count_DR,Likelihood+1)=rel_x;
...
foldername=[Bigname,'/NSR=',num2str(NSR),',obj=',num2str(Likelihood)];
mkdir(foldername)

figure
semilogy(1:count_DR-1,relative_DR_y(1:count_DR-1,Likelihood+1),'g')
hold on 
semilogy(1:count_DR-1,resi_DR_y(1:count_DR-1,Likelihood+1),'g:')
hold on 
xlabel('Epoch')
ylabel(' ')
name=[foldername,'/convergence'];
legend('RelErr','Residual')
saveas(gcf,name,'png')
close
...
close
    ...
    Rec_Err(count_COER,Likelihood+1)=rel_x;
    end
    NSR_Vec(count_COER)=NSR;
    
    
    
    
    
    count_COER=count_COER+1;

    end

figure
plot(NSR_Vec,Rec_Err(:,1),'ro-','LineWidth',2,...
                       'MarkerEdgeColor','k',...
                       'MarkerFaceColor','r',...
                       'MarkerSize',12)
hold on
plot(NSR_Vec,Rec_Err(:,2),'b^-','LineWidth',2,...
                       'MarkerEdgeColor','k',...
                       'MarkerFaceColor','w',...
                       'MarkerSize',12)
hold on
plot(NSR_Vec,NSR_Vec,':')
grid on
