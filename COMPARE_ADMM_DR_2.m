...
compare_iter_ADMM=zeros(length(Gamma));
 ...
 mkdir([Bigname,'/ADMM_2'])
 mkdir([Bigname,'/DR_2'])
 mkdir([Bigname,'/ADMM_DR_2']);
for gamma=Gamma
    residual_DR_y=zeros(MaxIter);
    residual_DR_lambda=zeros(MaxIter);
    residual_ADMM_y=zeros(MaxIter);
    residual_ADMM_z=zeros(MaxIter);
    %%%%%%%%%%%%%%
    trial=trial+1;
    %%%%%%%%%%%%%%
    lambda_t=lambda_0;
 ...
 %gamma=1.2;
    ...
    Name=[Bigname,...
            '/DR_2/add_phase=',num2str(add_phase),'_gamma=',...
            num2str(gamma),...
        ...
        fclose(fid);
    semi=1;
    %image_print(Name,count,semi,residual_DR_y,dev, Lambda_2)
    count_DR=count;
    ...
    Name=[Bigname,...
            '/ADMM_2/add_phase=',num2str(add_phase),'_gamma=',...
            num2str(gamma),...
            '_Noiseswitch=',num2str(Noiseswitch)];
    ...
    hold on
    semilogy(1:count-1,residual_ADMM_y(1:count-1),'r')
    hold on
    semilogy(1:count_DR-1,residual_DR_lambda(1:count_DR-1),'g-.')
    hold on
    semilogy(1:count-1,residual_ADMM_z(1:count-1),'r-.')
    title(['gamma= ',num2str(gamma)])
    xlabel('Iter')
    ylabel('|||y/z/lambda_{k+1}|-b||_2/||b||_2')
    axis([0,max(count,count_DR) 1e-12 1]);
    name=[Bigname,'/ADMM_DR_2/admm_dr_same_gamma=',num2str(gamma*10)];
    legend('DR y_k','ADMM y_k','DR lambda_k','ADMM z_k')
    saveas(gcf,name,'png')
    close
    %%%%%%%%%%%%%%%%%%%%%%%
