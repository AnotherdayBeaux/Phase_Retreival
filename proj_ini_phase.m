...
o_l = 0.7;%0.2:0.1:0.8 ; % overlaping
cim_diff=43; %20:15:120;
phase_reg =1;
...
%   num_mask,coer,SUBIM, type_im,l_patch,x_c_p, y_c_p,bd_con) 
    ...
    type_im,l_patch,x_c_p,y_c_p,bd_con,gamma_phi);
    ...
    for i = 3:10
    Bigname=['/Users/Beaux/Desktop/phase retrieval/code/16/linear phase/f_rank_perb_linearmaskini/',num2str(i)];
    mkdir(Bigname);
    lambda_t=lambda_0;
...
while rel_x > Tol && count_DR< MaxIter
   ...
   % calculate nor_phase dependent on image
    ...
    colorbar
    title('colormapIM')
    name=[Bigname,'/ColormapIMDifference'];
    %saveas(gcf,name,'png')
    %close
    ...
    name=[Bigname,'/ColormapIni-MaskDifference'];
    %saveas(gcf,name,'png')
    ...
    colorbar
    title('colormapMASK')
    name=[Bigname,'/ColormapMaskDifference'];
    %saveas(gcf,name,'png')
