...
gamma=1;
relative_DR_y=zeros(MaxIter,2,3);
...
% patch.
    ...
    Bigname='/Users/Beaux/Desktop/phase retrieval/code/19/bp_paper_figure/diff_mask_type';
    mkdir(Bigname)
...
while rel_x > Tol && count_DR< MaxIter
   ...
   % calculate nor_phase dependent on image
    ...
    relative_DR_y(count_DR,Likelihood+1,mask_type+1)=rel_x;
...
Count_DR(likelihood+1,mask_type+1)=count_DR;



foldername=[Bigname,'/masktype=',num2str(mask_type),',obj=',num2str(Likelihood)];
mkdir(foldername)

figure
semilogy(1:count_DR-1,relative_DR_y(1:count_DR-1,Likelihood+1,mask_type),'g')
hold on 
...
figure
semilogy(1:Count_DR(1,0)-1,relative_DR_y(1:1:Count_DR(1,0)-1,1,1),'r')
hold on
semilogy(1:Count_DR(1,0)-1,relative_DR_y(1:1:Count_DR(1,0)-1,1,2),'g')
hold on
semilogy(1:Count_DR(1,0)-1,relative_DR_y(1:1:Count_DR(1,0)-1,1,3),'b')
legend('i.i.d','Correlated','Fresenl')
xlabel('Epoch')
ylabel('Err')

name=[Bigname,'/Error_pois_compare'];
saveas(gcf,name,'png')
close