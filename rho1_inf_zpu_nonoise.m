...
Rec_err=zeros(len_RS,len_RHO2);   % reconstruction error
...
Bigname='/Users/Beaux/Desktop/phase retrieval/code/13/rho1-->inf';
mkdir(Bigname);
trial_RS=1;
    ...
    trial_RHO2=1;
        ...
        while (rel_up > rel_u && count_DR < MaxIter) || count_DR<100
            ...
            % update u
             ...
             divplambda1=div_p(p_k);
             %b=lap\divplambda1(:);
             u_kplus1=ifft2(fft2(divplambda1)./w1w2Matrix);
        ...
        Rec_err(trial_RS,trial_RHO2)=rel_u;
        ...
        xlabel('interation')
        ylabel('rel_u')
        Title=['rs=',num2str(rs),',rho2=',num2str(rho2)];
        title(Title)
...
save('Rec_err.mat','Rec_err','RHO2','RS')
...
for k=1:len_RS
    ...
    ylabel('rho1')
    zlabel('rel err')
    loc=['/Users/Beaux/Desktop/phase retrieval/code/13/rho1-->inf',Title];
    saveas(gcf,loc,'png')
