...
Bigname='/Users/Beaux/Desktop/phase retrieval/code/10/no_random_sample';
mkdir(Bigname);
trial=1;
...
for alpha=ALPHA 
    ...
    for rho2=RHO2
        ...
        Rec_err(trial,trial_RHO2,trial_ALPHA)=-10* log(rel_u^2)/log(10);
        ...
        xlabel('interation')
        ylabel('rel_u')
        Title=['SNR int=',num2str(SNR_DB),',alpha=',num2str(alpha),',rho2=',num2str(rho2)];
        title(Title)
...
for k=1:l_d
    ...
    [x,y]=meshgrid(ALPHA,RHO2);
    figure
    %mesh(x,y,a);
    plot3(x,y,a,'+');
    grid on
    Title=['SNR intensity=', num2str(SNR_DB)];
    title(Title)
    xlabel('alpha')
    ylabel('rho2')
    zlabel('SNR DB')
    loc=['/Users/Beaux/Desktop/phase retrieval/code/10/no_random_sample/',Title];
    saveas(gcf,loc,'png')
    close
