...
%%%%%% Find the importance of overlaping region
...
O_L = 0.2:0.1:0.8 ;
CIM_DIFF=20:15:120;
Final_Error=zeros(length(CIM_DIFF),length(O_L));
...
% l_patch  =side length of patch      regulates the size of each small patch.
count_CIM=1;
for cim_diff = CIM_DIFF
    ...
    mkdir(Bigname);
    x_line=1:cim_diff:256; y_line=1:cim_diff:256;
    x_c_p=(1:cim_diff:256)'*ones(1,length(x_line));
    y_c_p=ones(length(x_line),1)*(1:cim_diff:256);
...
while rel_x > Tol && count_DR< MaxIter
    ...
    MaxIter_u_ip=(count_DR < 20)*120+...
        (count_DR >= 20 && count_DR < 40 )* 60 +...
        (count_DR>= 40 && count_DR<60) *40+...
        (count_DR >= 60 ) *5;
   ...
   % calculate nor_phase dependent on image
    ...
    legend('rel','resi')
    subname=['num sub im=',num2str(num_subim),'patch size=',num2str(l_patch)];
    name=[Bigname,'/',subname];
    title(subname)
    saveas(gcf,name,'png')
    close
    ...
    Final_Error(count_CIM,count_OL)=rel_x;
    
    
    count_OL=count_OL+1;
    
    end
    count_CIM=count_CIM+1;
end
NUM_IM=(floor((256./CIM_DIFF)+1)).^2;
[X,Y]=meshgrid(NUM_IM,O_L);

mesh(X,Y,Final_Error);

xlabel('num im')
ylabel('patch size')
name='/Users/Beaux/Desktop/phase retrieval/code/16/diff_patch/Final_Error';
saveas(gcf,name,'png')
